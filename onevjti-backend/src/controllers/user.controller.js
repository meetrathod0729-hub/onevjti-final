...
const getCurrentUser = asyncHandler(async (req, res) => {
    ...
    if (member) {
      role = member.role; // core / head / admin
    }
    ...
    return res.status(200).json(
      new ApiResponse(200, userWithRole, "Current user fetched")
    );
  });
...
const updateAvatar = asyncHandler(async (req, res) => {
    ...
    if (!req.file) {
        return res.status(400).json(
            new ApiResponse(400, null, "No avatar file uploaded")
        );
    }
    ...
    if (!avatar || !avatar.secure_url) {
        return res.status(500).json(
            new ApiResponse(500, null, "Cloudinary upload failed")
        );
    }
